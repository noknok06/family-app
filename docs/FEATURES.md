...
- 一覧のカードと詳細モーダルに参考画像（`places/PlacePhoto`）を表示。画像は登録させず、登録済みの場所名・住所から Google の場所写真を引いて出す。画面に入ったカードだけ取得し、取得結果は端末に短期キャッシュ、1 日 / 1 か月の呼び出し上限付き（無料枠内で運用）。写真が無い場所は従来どおりの表示（詳細ではカテゴリアイコンのプレースホルダー）
...
- 旅行の計画要素: 期間・都道府県・同行者・交通手段・宿泊先・予算・メモ。作成時に予定表へ自動登録（既存どおり）。開始日を選ぶと終了日が未入力・開始日より前のときは開始日にそろう
...
- 一覧カードには状態チップに加えて、出発前は「あと◯日」、旅行中は「◯日目 / 全◯日」、終わった旅行は「◯日間」を表示
- 詳細モーダル（`travel/TripDetailModal`）は「概要 / 準備 / 行程」の 3 タブ。タブバーはモーダルのヘッダー直下に固定されるので、行程を下まで送ってもタブを切り替えられる。旅行中は「行程」タブから開く
  - 概要: 計画情報の一覧と、予算に対する行程費用の合計・残り（超過時は赤字表示）、参加人数で割った 1 人あたり予算・費用
  - 準備: 持ち物 / やること のチェックリスト（担当者付き・進捗メーター）。`travel_prep_items`。追加フォームは進捗メーターの直下に置き、担当は家族メンバーから選ぶ（メンバーがいない家族は自由入力）。削除は「元に戻す」付きの通知で取り消せる
  - 行程: 日ごとに区切ったスケジュール（時刻・場所・費用・メモ）。当日はチェックで実行済みにできる。日ごとの見出しはタブバーの下に固定表示され、旅行中は今日の日を強調して開いたときにそこまでスクロールする
